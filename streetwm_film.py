...
class ACPredictor(nn.Module):
    def __init__(self, enc_dim=ENC_DIM, pred_dim=PRED_DIM, depth=PRED_DEPTH,
                 ...
                 n_patches=N_PATCHES, patch_h=PATCH_H, patch_w=PATCH_W, max_seq=32):
        super().__init__()
...
class WorldModelGame:
    def __init__(self, root):
        ...
        # Set baseline resolution (e.g., 960x640)
    ...
    def _build_ui(self):
        ...
        # ---- Sidebar contents ----
        P = dict(padx=12)
        tk.Label(sidebar, text="World Model", bg=PANEL, fg=BRIGHT, font=('Arial', 11, 'bold')).pack(anchor='w', pady=(14, 0), **P)
        ...
        self._sep(sidebar)
        tk.Label(sidebar, text="Stats", bg=PANEL, fg=DIM, font=('Arial', 9)).pack(anchor='w', pady=(8, 4), **P)
    ...
    def _finalize_video(self):
        ...
        # FFmpeg command: 10 fps, libx264 codec, yuv420p for compatibility
        cmd = [
            'ffmpeg', '-y',
            '-framerate', '10',
        ...
        # Cleanup
        shutil.rmtree(self.record_dir)
        self.root.after(0, lambda: self.rec_btn.configure(text="⏺ Start Recording", fg=ACCENT, state='normal'))
    ...
    def _step(self, dx, dy):
        ...
        with torch.no_grad():
            nxt = self.predictor(self.latent_history[:, -1:], action)
            self.latent_history = torch.cat([self.latent_history, nxt], dim=1)[:, -8:]
            recon = self.decoder(nxt.squeeze(1))
        ...
        scale = min(vw/256, vh/256) * 0.9 # 90% of max size to keep it "smaller" as requested
        target_size = (int(256 * scale), int(256 * scale))
        
        if target_size[0] > 10:
            pil_disp = pil.resize(target_size, Image.Resampling.BILINEAR)
            self._show(pil_disp)
    ...
    def _load_models(self):
        def _load():
            self._status("Loading V-JEPA 2 encoder (CPU)…")
            self.encoder = AutoModel.from_pretrained(
                'facebook/vjepa2-vitl-fpc64-256', torch_dtype=torch.float16
            )  # stays on CPU
            self.encoder.eval()
            ...
            for name, model, ckpt in [
                ('Predictor', self.predictor, '45000_aftwalk_r4_FINAL_val_tf_0_5368_val_roll_0_6805.pt'),
                ('Decoder',   self.decoder,   'decoder_v3.pt'),
            ]:
    ...
    def _load_start_image(self):
        ...
        tensor = tf(img).unsqueeze(0).unsqueeze(1).to(self.device).half()

        self._status("Encoding initial frame…")
        self.encoder.to(self.device)  # CPU -> VRAM
        with torch.no_grad():
            out = self.encoder(pixel_values_videos=tensor, skip_predictor=True)
            self.latent_history = out.last_hidden_state.unsqueeze(1).half()
        self.encoder.to('cpu')        # VRAM -> CPU
    ...
    def _refresh_stats(self):
        ...
        self.stat_fps.configure(text=fps_str)
